...
H_mag = np.sqrt(np.square(np.abs(Hz)))
...
minimize(intensity, eps_r, args=(), method='L-BFGS-B', jac=grad_I,
    ...
    options={'disp': True, 'maxcor': 10, 'ftol': 2.220446049250313e-09, 'gtol': 1e-05, 'eps': 1e-08, 'maxfun': 15000, 'maxiter': 15000, 'iprint': -1, 'maxls': 20})
...
Ex, Ey, Hz = F.solve()
plt.imshow(np.real(Hz._value), cmap='RdBu')
plt.title('H_z / E0 (<-)')
plt.xlabel('y')
plt.ylabel('x')
plt.colorbar()
